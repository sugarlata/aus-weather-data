...
@pytest.mark.parametrize(
    "radar_id, radar_type",
    [
        (radar_location, radar_type) for radar_location in BOMRadarLocation.IDR_LIST for radar_type in [t for t in RADAR_TYPE]
    ]
)
def test_matching_files_normal(radar_id, radar_type):
    ...
    for metadata in matching_files:
        assert metadata.radar_id == radar_id
